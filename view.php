...
// echo $OUTPUT->single_button(new moodle_url('https://www.yandex.ru/'), 'Яндекс', 'post');
...
$all_mistake = 0;
foreach ($obj as $obj) {
    $timeOffset += (int)$obj->time_offset;
    $mistake += (int)$obj->mistake; 
    $all_mistake += (int)$obj->mistake;
    if ($obj->mistake != 1) {
        $timeOffset = number_format((float)($timeOffset / 1000), 2, '.', '');
        $table->data[] = array($obj->frame_number, $timeOffset, $mistake);  
        $all_time += $timeOffset;
        $timeOffset = 0;
        $mistake = 0;
     }
...
// Выполняем запрос
$result = $DB->get_records('user_chapters');
$t = new html_table();
$t->head = array('Номер шага','Время, сек');
foreach ($result as $res) {
    $t -> data[] = array($res->chapter_id, $res->login);
}
