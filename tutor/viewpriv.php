...
$PAGE->set_url('/mod/tutor/viewpriv.php', array('user' => $user->id));
$PAGE->set_title(format_string($user->firstname));
$PAGE->set_heading(format_string($user->firstname));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('privatesession', 'tutor', $user->firstname));
if ($USER->id == $user->id) {
    $moderation = 1;
} else {
    $moderation = 0;
...
echo $OUTPUT->box(get_string('instruction', 'tutor'));
echo $OUTPUT->single_button(new moodle_url('/mod/tutor/sessionpriv.php', $urlparams), get_string('access', 'tutor'), 'post');
