...
class update_session_log extends \core\task\scheduled_task
{
    ...
    public function execute()
    {
        ...
        $query = sprintf("SELECT u.firstname, u.lastname, u.email
            FROM mdl_role_assignments AS ra
            JOIN mdl_context AS context ON context.id = ra.contextid AND context.contextlevel = 50
            JOIN mdl_course AS c ON c.id = context.instanceid AND c.id = 2
        ...
        foreach ($result as $res) {
            $newrecordcourse = new \stdClass();
            $newrecordcourse->chapter_id = $res->firstname;
            $newrecordcourse->login = $res->email;
            $d[] = $newrecordcourse;
        }
        $DB->insert_records('user_chapters', $d);
