...
$PAGE->set_url($CFG->wwwroot.'/mod/tutor/session.php');
...
$PAGE->set_title(get_string('privatesession', 'tutor', $user->firstname));
$PAGE->set_heading(get_string('privatesession', 'tutor', $user->firstname));
echo $OUTPUT->header();
...
$payload  = json_encode([
  ...
  "aud" => "tutor",
  "iss" => $CFG->tutor_app_id,
  "sub" => $CFG->tutor_domain,
...
$secret = $CFG->tutor_secret;
$signature = hash_hmac('sha256', $base64urlheader . "." . $base64urlpayload, $secret, true);
$base64urlsignature = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($signature));

$jwt = $base64urlheader . "." . $base64urlpayload . "." . $base64urlsignature;
echo "<script src=\"https://".$CFG->tutor_domain."/external_api.js\"></script>\n";

echo "<script>\n";
echo "var domain = \"".$CFG->tutor_domain."\";\n";
echo "var options = {\n";
echo "configOverwrite: {\n";
echo "channelLastN: ".$CFG->tutor_channellastcam.",\n";
echo "startWithAudioMuted: true,\n";
echo "startWithVideoMuted: true,\n";
echo "},\n";
echo "roomName: \"".urlencode($sessionnorm)."\",\n";

if ($CFG->tutor_app_id != null && $CFG->tutor_secret != null) {
    echo "jwt: \"".$jwt."\",\n";
}
if ($CFG->branch < 36) {
...
if ($teacher == true && $CFG->tutor_livebutton == 1) {
...
$youtubeoption = '';
if ($CFG->tutor_shareyoutube == 1) {
    $youtubeoption = 'sharedvideo';
}

$bluroption = '';
if ($CFG->tutor_blurbutton == 1) {
    $bluroption = 'videobackgroundblur';
}

$security = '';
if ($CFG->tutor_securitybutton == 1) {
    $security = 'security';
}

$invite = '';
if ($CFG->tutor_invitebuttons == 1) {
    $invite = 'invite';
}
...
echo "SHOW_tutor_WATERMARK: true,\n";
echo "tutor_WATERMARK_LINK: '".$CFG->tutor_watermarklink."',\n";
echo "},\n";

echo "width: '100%',\n";
echo "height: 650,\n";
echo "}\n";
echo "var api = new tutorMeetExternalAPI(domain, options);\n";
echo "api.executeCommand('displayName', '".$nombre."');\n";
echo "api.executeCommand('avatarUrl', '".$avatar."');\n";
if ($CFG->tutor_finishandreturn == 1) {
